...
# mesh = meshio.read(r"horn_parab_4.msh")
...
#"Elec_field/HORNANTENNA_PARABOLIC REFLECTOR/HORN_180_LARGERPLANE.txt",
###########################################################################
surface1_measurements = pd.read_csv(
    r"Elec_field/CST_DATA/Dx13_s250.txt",
    sep=r"\s+",
    skiprows=[0,1],
    engine="python",
...
surface2_measurements = pd.read_csv(
    r"Elec_field/CST_DATA/Dx13_s300.txt",
    sep=r"\s+",
    skiprows=[0,1],
    engine="python",
    names=["x_mm", "y_mm", "z_mm",
...
def armijo_line_search(un, vn, gn, alpha_init=1.0, rho=0.5, c_armijo=1e-4, max_iter=50):
